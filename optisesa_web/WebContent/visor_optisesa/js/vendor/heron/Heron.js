...
Heron.widgets.search.FeatureInfoPanel = Ext.extend(Ext.Panel, {
    ...
    handleBeforeGetFeatureInfo: function(evt) {
        ...
        if (this.olControl.layers.length == 0) {
            ...
            for (var index = 0; index < this.map.layers.length; index++) {
                layer = this.map.layers[index]; /*if(!layer instanceof OpenLayers.Layer.WMS||!layer.params){continue;}*/
                if (layer.visibility && layer.params && !layer.isBaseLayer/*&&(layer.featureInfoFormat||layer.params.INFO_FORMAT)*/ ) {
                    /*if(!layer.params.INFO_FORMAT&&layer.featureInfoFormat){layer.params.INFO_FORMAT=layer.featureInfoFormat;}
    ...
    displayFeatureInfo: function(evt) {
        ...
        for (featureSetKey in featureSets) {
            ...
            for (var c1 = 0; c1 < this.olControl.layers.length; c1++) {
                if (this.olControl.layers[c1].name == featureSet.title && this.olControl.layers[c1].idfields) {
                	var arrCol = this.olControl.layers[c1].idfields.split(","),
                	    arrColName = this.olControl.layers[c1].idnamefields.split(",");;
                	for (var j = 0; j < arrCol.length; j++) {
                		var objCol = {
                          header: arrColName[j],
                          dataIndex: arrCol[j]
                      }
                      columnsLayer.push(objCol);	
                	}
                    break;
                }
            }

            if(columnsLayer.length>0)
        ...
        var options = {
            ...
            listeners: {
                ...
                checkchange: function(node, checked) {
                    ...
                    if(checked)
                            Heron.App.map.getControlsByClass("OpenLayers.Control.WMSGetFeatureInfo")[0].activate();
                },
                scope: this
            }
        };
    ...
    layerFilter: function(map) {
        return map.getLayersBy('metadata', {
            test: function(metadata) {
                return metadata && metadata.wfs && metadata.wfs.protocol && !metadata.isSourceLayer;
            }
        })
    },
...
Heron.widgets.search.SearchByDrawPanel = Ext.extend(Heron.widgets.search.SpatialSearchPanel, {
    ...
    initComponent: function() {
        this.tLabelLayer = new Ext.form.Label({
            html: __('LabelLayer'),
            style: 'labelsearchdraw'
        });
        this.tLabelTool = new Ext.form.Label({
            html: __('LabelTool'),
            style: 'labelsearchdraw'
        });
        this.tLabelSearch = new Ext.form.Label({
            html: __('LabelSearch'),
            style: 'labelsearchdraw'
        });
        this.items = [this.tLabelLayer, this.createTargetLayerCombo(), this.tLabelTool, this.createDrawToolPanel(), this.tLabelSearch, this.createStatusPanel(), this.createActionButtons()];
...
Heron.widgets.ToolbarBuilder.defs = {
    ...
    featureinfo: {
        ...
        create: function(mapPanel, options) {
            ...
            Heron.widgets.ToolbarBuilder.onActiveGetFeatureInfo= function() {
                $(".x-btn .icon-getfeatureinfo").css("background-image","url('images/silk/information_sel.png')");
            };

            Heron.widgets.ToolbarBuilder.onDeActiveGetFeatureInfo= function(){
                //Ext.Msg.alert(__('Warning'),"desactivado");
            ...
            options.control.events.register("activate", map, Heron.widgets.ToolbarBuilder.onActiveGetFeatureInfo);
            options.control.events.register("deactivate", map, Heron.widgets.ToolbarBuilder.onDeActiveGetFeatureInfo);
    ...
    zoomvisible: {
        ...
        create: function(mapPanel, options) {
            options.handler = function() {
                Heron.App.map.zoomToExtent(Heron.App.map.maxExtent);
            };
            return new GeoExt.Action(options);
        }
    },
