...
Ext.ux.grid.FilterRow = Ext.extend(Ext.util.Observable, {
  ...
  init: function(grid) {
    this.grid = grid.tableGrid;
    this._timeout=null;
    var cm = grid.tableGrid.getColumnModel();
    var view = grid.tableGrid.getView();
    grid.tableGrid.id=grid.id+"_tableGrid";
    /*
    ...
    grid.tableGrid.addClass('filter-row-grid');
    
    // when grid initially rendered
    grid.tableGrid.on("render", this.renderFields, this);
    
    // when Ext grid state restored (untested)
    grid.tableGrid.on("staterestore", this.resetFilterRow, this);
    
    // when the width of the whole grid changed
    grid.tableGrid.on("resize", this.resizeAllFilterFields, this);
  ...
  onFieldChange: function() {
    ...
    if (this.autoFilter) {
      if(!this.grid.getStore())
        this.grid.store=featureGridRS.items.items[0].store;
  ...
  getFilterFunction: function() {
    var tests = [],strategiesLayer,layer=this.grid.store.layer;
    if(this.grid.store.layer.strategies){
      if(this.grid.store.layer.strategies.length>1)
        strategiesLayer=this.grid.store.layer.strategies[1];
      else
        strategiesLayer=this.grid.store.layer.strategies[0];
    }else{
      this.grid.store.layer.strategies= [new OpenLayers.Strategy.Filter({
    ...
    this.eachFilterColumn(function(col) {
      var p = col.filter.createPredicate(col.dataIndex,strategiesLayer,layer);
      if (p) {
        tests.push(p);
      }
    });
...
Ext.ux.grid.FilterRowFilter = Ext.extend(Ext.util.Observable, {
  ...
  constructor: function(config) {
    ...
    if (!this.field) {
      if (this.type=="string") {
        ...
        this.field = new Ext.ux.form.SearchField({enableKeyEvents: true}); 
      } 
   ...
   * @return {Function}
   */
  createPredicate: function(dataIndex,filterLayerStrategy,layer) {
    var test = this.test,aplicaFiltro=false,filterValue = this.field.getValue();
    // is test a regex string?
    if (typeof test === "string" && test.match(/^\/.*\/[img]*$/)) {
      return this.createRegExpPredicate(test, filterValue, dataIndex,filterLayerStrategy,layer);
    }
    else {
      ...
      {
          var bandera=true,filterType=null,valorFiltro=0,lowerBoundary=0,upperBoundary=0;

          var comparables = filterValue.replace(/\ /g,"");
          //console.log(comparables);
          comparables=comparables.split("y");
          if(comparables.length===2)
          {
              if(comparables[0].substring(0,1)===">"&&comparables[1].substring(0,1)==="<"){
                  filterType=OpenLayers.Filter.Comparison.BETWEEN;
                  lowerBoundary=Number(comparables[0].substring(1));
                  upperBoundary=Number(comparables[1].substring(1));
              }
          }else {
              if(filterValue.substring(0,1)===">"){
                  filterType = OpenLayers.Filter.Comparison.GREATER_THAN;
                  valorFiltro = Number(filterValue.substring(1));
              }else if (filterValue.substring(0,1)==="<") {
                  filterType = OpenLayers.Filter.Comparison.LESS_THAN;
                  valorFiltro = Number(filterValue.substring(1));
              }else {
                  filterType = OpenLayers.Filter.Comparison.EQUAL_TO;
                  valorFiltro = Number(filterValue);
              }
          }

          if(filterType){
            if(!filterLayerStrategy.active){
              filterLayerStrategy.layer=layer;
              filterLayerStrategy.activate();
            }

            $.each(filterLayerStrategy.filter.filters, function(index, data) {
              if(data.property===dataIndex){
                data.type= filterType;
                if(filterType===OpenLayers.Filter.Comparison.BETWEEN){
                  data.lowerBoundary = lowerBoundary;
                  data.upperBoundary=upperBoundary;
                }
                else
                  data.value = valorFiltro;
                aplicaFiltro=true;
                bandera=false;
                return;
              }
            });
            if(bandera && filterValue){
              if(filterType===OpenLayers.Filter.Comparison.BETWEEN){
                filterLayerStrategy.filter.filters.push(new OpenLayers.Filter.Comparison({
                                        type: filterType,
                                        property: dataIndex,
        ...
        $.each(filterLayerStrategy.filter.filters, function(index, data) {
              if(data.property===dataIndex){
                data.type= OpenLayers.Filter.Comparison.LIKE;
                data.value = "";
                aplicaFiltro=true;
                return;
              }
      ...
      if(aplicaFiltro)
              filterLayerStrategy.setFilter(filterLayerStrategy.filter);
      return function(r) {
  ...
  createRegExpPredicate: function(reString, filterValue, dataIndex, filterLayerStrategy, layer) {
    // don't filter the column at all when field is empty
    var bandera=true;
    var aplicaFiltro=false;
    if(!filterLayerStrategy.active){
      filterLayerStrategy.layer=layer;
      filterLayerStrategy.activate();
    }
